...
Playwright 会自动启动 Vite（端口 4173），串行验证 Dashboard、编辑器审批配置、双区域校验、条件修复、动态表单预览、发布联动、异常实例详情、版本比较和历史恢复，以及按业务域分批试点（停批/重开、版本路由、记录持久化）。测试失败时会保留截图和 trace。
...
9. 在运行监控的“分批试点发布”控制台停批“财务”域：该域后续新申请回现网 v1，其他试点域照常走 v2，办理中实例版本不变。
10. 打开表单预览，切换申请业务域（试点/停批/未试点），观察路由提示，模拟提交后在监控中核对执行版本与试点状态。
11. 在控制台重新开放停批域并指定版本（v1/v2），刷新页面确认试点分配与停批记录仍在。

## 项目结构

- `mock-data/`：12 个流程、80 个实例、8 个用户、6 个角色、5 个业务域及边界案例。
- `src/components/`：布局、公共状态组件、分批试点控制台和 React Flow 画布。
